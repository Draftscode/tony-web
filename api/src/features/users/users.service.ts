...
} from '../../common/contracts/user-repository.interface';
...
export class UsersService implements IUserRepository {
  ...
  async initializeApp() {
    try {
      await this.datasource.manager.transaction(async (manager) => {
        let e = await manager.findOne(UserEntity, {
        ...
        e.lastname = '';
        e.roles = [
          await manager.findOneOrFail(RoleEntity, { where: { name: 'admin' } }),
        ];
  ...
  async getAll(query: string = '') {
    const [items, total] = await this.datasource.manager.findAndCount(
      UserEntity,
      { where: { username: ILike(`%${query}%`) }, relations: { users: true } },
    );
    return { items, total };
  }
  ...
  async editUser(
    id: number,
    user: Partial<UserEntity> & { newPassword?: string },
  ) {
    return this.datasource.manager.transaction(async (manager) => {
      if (user.roles !== undefined) {
        user.roles = await this.roleRepo.findByIds(
          user.roles?.map((r) => r.id) ?? [],
        );
      }

      if (user.brokers !== undefined) {
        user.brokers = await this.brokerRepo.findByIds(
          user.brokers?.map((r) => r.id) ?? [],
        );
      }
      ...
      if (newPassword) {
        payload.password = encodePassword(newPassword);
      }

      return manager.save(UserEntity, payload);
    });
  }
}
