...
export class FilesService {
  ...
  async getAll(
    ...
    const qb = repository
      ...
      .leftJoin(
        MessageEntity,
        'message',
        `message.userId = :userId AND message.topic = :topic AND message.entityId = file.id`,
        { userId: user.id, topic: 'file' },
      )
  ...
  async removeFile(filename: string) {
    return this.dataSource.transaction(async (manager) => {
      const file = await manager.findOne(FileEntity, { where: { filename } });
      if (!file) { return; }
      await manager.delete(NoteEntity, { fileId: file.id });
      await manager.delete(MessageEntity, { topic: 'file', entityId: file.id });
      return manager.delete(FileEntity, { id: file.id });
    });
  }
  ...
  async createPdf(contents: string, options?: Partial<{ logo: string }>) {
    ...
    try {
      ...
      await page.setContent(usePdfPreset(options?.logo, contents), {
