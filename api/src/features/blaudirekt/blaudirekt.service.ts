...
function splitStreetAndNumber(fullStreet?: string): {
  street: string;
  streetNo: string;
} {
...
export class BlaudirektService {
  ...
  async fetchCompanies(
    ...
    try {
      const response = await lastValueFrom(
        this.httpService.get(`https://stocks.ameiseapis.com/api/companies`, {
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
  ...
  async fetchBrokers(
    ...
    try {
      const response: AxiosResponse<BlaudirektResponse<BrokerEntity>> =
        await lastValueFrom(
          this.httpService.get(`https://stocks.ameiseapis.com/api/brokers`, {
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded',
              Authorization: `Bearer ${token}`,
            },
      ...
      await this.createOrUpdateBrokers(
        response.data.items as QueryDeepPartialEntity<BrokerEntity>[],
      );
      return response.data;
    } catch (e) {
      this.logger.error(e);
  ...
  async fetchCustomers(
    ...
    try {
      const response: AxiosResponse<BlaudirektResponse<BlaudirektCustomerDto>> =
        await lastValueFrom(
  ...
  async editCompany(
    companyId: string,
    companyDto: QueryDeepPartialEntity<CompanyEntity>,
  ) {
  ...
  async editDivision(
    divisionId: string,
    divisionDto: QueryDeepPartialEntity<DivisionEntity>,
  ) {
    return this.dataSource.manager.transaction(async (manager) => {
  ...
  async getContracts(
    ...
    return this.dataSource.manager.find(ContractEntity, {
      where: { customerId },
      skip: offset,
      take: limit,
      relations: { customer: true, company: true },
    });
  }

  async ask<Body extends BodyInit | null | undefined, T>(
    path: string,
    config?: Partial<{
      method: string;
      body: Body;
  ...
  async addDocument(customerId: string, html: string) {
    ...
    const result = await this.ask(`${GRANT.sub}/archiveintraege`, {
      ...
      headers: {
        ...
        'X-Dio-Metadaten': JSON.stringify([
          { Value: '_kundensichtbar', Text: '1' },
        ]),
        'X-Dio-Typ': 'dokument',
  ...
  async getCustomers(
    options: Partial<SearchOptions & { status?: string; brokers: string[] }>,
  ) {
    ...
    if (options?.query) {
      qb.where('customer.displayName ILIKE :query', {
        query: `%${options.query}%`,
      });
    }
    ...
    if (Array.isArray(options.filters?.status)) {
      ...
      const statusValues = options.filters.status.flatMap(
        (status) => status.value,
      );
    ...
    if (options?.brokers?.length) {
      qb.andWhere('customer."brokerId" IN (:...brokerIds)', {
        brokerIds: options.brokers,
      });
    }

    qb.skip(options?.offset ?? 0).take(options?.limit ?? 100);
  ...
  private createOrUpdateCustomers(
    broker: BrokerEntity,
    customers: BlaudirektCustomerDto[],
  ) {
    const items: QueryDeepPartialEntity<CustomerEntity>[] = customers
      ...
      .map((customer) => {
        return {
          ...
          mainAddress: {
            ...
            streetNo: splitStreetAndNumber(customer.mainAddress.street)
              ?.streetNo,
          },
  ...
  private async createOrUpdateContracts(contracts: Record<string, any>[]) {
    const companyIds: number[] = [
      ...new Set<number>(contracts.map((c) => c.company?.id).filter(Boolean)),
    ];

    const existingCompanies = await this.dataSource.manager.find(
      CompanyEntity,
      {
        select: ['id'],
  ...
  private createOrUpdateBrokers(
    brokers: QueryDeepPartialEntity<BrokerEntity>[],
  ) {
    return this.dataSource.transaction(async (manager) => {
  ...
  private createOrUpdateCompanies(companies: Partial<CompanyEntity>[]) {
    const mappedCompanies: QueryDeepPartialEntity<CompanyEntity> = companies
      ?.filter((company) => !!company.id)
      .map(
        ({ contracts, ...rest }) => rest,
      ) as QueryDeepPartialEntity<CompanyEntity>;
