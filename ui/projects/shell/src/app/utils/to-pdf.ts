...
export function toPdf(content: Content, translate: TranslateService, color: string | null = 'rgb(25, 66, 109)') {
    ...
    const styles = `
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .address-card .line { color: #4b5563; }
        ...
        tr { break-inside: avoid; page-break-inside: avoid; -webkit-column-break-inside: avoid; }
        ...
        .badge-acquisition{ background: #fef3c7; color: #92400e; }
        .badge-default    { background: #f3f4f6; color: #374151; }

        /* ── Summary rows ──────────────────────────────────── */
        tr.summary-old td, tr.summary-new td, tr.summary-savings td {
            padding: 5px 7px;
            font-size: 10px;
        }
        tr.summary-old {
        ...
        tr.summary-old td { color: #6b7280; }
        tr.summary-new { background: #eff6ff; }
        tr.summary-new td { color: #1e40af; }
        tr.summary-savings { background: #ecfdf5; }
        tr.summary-savings td { color: #065f46; font-weight: 700; }
...
`
    ...
    content.groups.forEach((group) => {
        ...
        group.items.forEach((row) => {
            ...
            if (!row['monthly']) contrib = contrib / 12;

            const img = row['insurer']?.logo ? row['insurer'].logo : defaultImage;
            const suggVal = row['suggestion']?.value ?? '';
            const suggLabel = row['suggestion']?.label ?? '-';

            const insurerCell = row['insurer']
                ? `<div class="insurer-cell"><img src="${img}" /><span>${row['insurer'].name}</span></div>`
                : '-';

            const badge = `<span class="badge ${badgeClass(suggVal)}">${suggLabel}</span>`;
            ...
            const cells = [
                ...
                `<td>${row['scope'] ?? '-'}</td>`,
                `<td>${row['fromTo'] ?? '-'}</td>`,
                `<td class="amount">${(row['oneTimePayment'] ?? 0)}€</td>`,
                `<td class="amount">${contrib.toFixed(2)}€</td>`,
            ].join('');

            rows += `<tr class="${_switch === 1 ? 'odd' : 'even'}">${cells}</tr>`;

            if (row['suggestion']?.value === 'terminated') { savings += contrib; existed += contrib; }
            if (row['suggestion']?.value === 'inventory')  { existed += contrib; current += contrib; }
            if (row['suggestion']?.value === 'new')        { totalOnce += suffix * Number.parseFloat(`${row['oneTimePayment']}`); savings -= contrib; current += contrib; }
            if (row['suggestion']?.value === 'acquisition'){ current += contrib; existed += contrib; }

            _switch *= -1;
        });

        const monthly = translate.instant('label.frequency.monthly');
        const table = `
        <div class="section-header">
            <span class="pill">${group.name}</span>
            <span class="line"></span>
        </div>
        <div class="table-wrapper"><table cellspacing="0" cellpadding="0">
        <tbody>
            <tr class="header-row">
                <td style="width:10%">${translate.instant('label.product')}</td>
                <td style="width:10%">${translate.instant('label.insurer.no')}</td>
                <td style="width:16%">${translate.instant('label.insurer.label')}</td>
                <td style="width:10%">${translate.instant('label.insured.short')}</td>
                <td style="width:10%">${translate.instant('label.suggestion')}</td>
                <td style="width:18%">${translate.instant('label.scope')}</td>
                <td style="width:10%">${translate.instant('label.fromTo')}</td>
                <td style="width:8%;text-align:right">${translate.instant('label.oneTimePayment')}</td>
                <td style="width:8%;text-align:right">${translate.instant('label.contribution.label')}</td>
            </tr>
            ${rows}
            <tr class="summary-old">
                <td colspan="7">${translate.instant('label.finances.old')} (${monthly})</td>
                <td class="amount"></td>
                <td class="amount">${existed.toFixed(2)} €</td>
            </tr>
            <tr class="summary-new">
                <td colspan="7">${translate.instant('label.finances.new')} (${monthly})</td>
                <td class="amount">${totalOnce.toFixed(2)} €</td>
                <td class="amount">${current.toFixed(2)} €</td>
            </tr>
            <tr class="summary-savings">
                <td colspan="7">${translate.instant('label.finances.savings')} (${monthly})</td>
                <td class="amount"></td>
                <td class="amount">${savings.toFixed(2)} €</td>
            </tr>
        </tbody>
        </table></div>`;
    ...
    content.persons.forEach(person => {
        ...
        const nameLine = [salutation, person.title, person.firstname, person.lastname].filter(Boolean).join(' ');

        addressCards += `
        <div class="address-card">
            ${person.company ? `<div class="line">${person.company}</div>` : ''}
            <div class="name">${nameLine}</div>
            <div class="line">${[person.street, person.streetNo].filter(Boolean).join(' ')}</div>
            <div class="line">${[person.zipCode, person.city].filter(Boolean).join(' ')}</div>
        </div>`;

        signatureBoxes += `
        <div class="signature-box">
            <div class="sig-label">${translate.instant('label.signature')}</div>
            <div class="sig-line">${person.firstname} ${person.lastname}</div>
        </div>`;
    });

    html += `
    <div class="address-block">${addressCards}</div>
    ${tables}
    <div class="signature-section">${signatureBoxes}</div>
`;
