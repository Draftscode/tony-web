...
export default class FormComponent {
    ...
    protected close(): void {
        this.router.navigate(['./..'], { relativeTo: this._ngActiveRoute });
    }
    ...
    protected moveTo(source: string, target: string, rowIndex: number) {

        const sourceGroup = this._formGroup.controls.groups.controls.find(group => group.value.name === source);
        const targetGroup = this._formGroup.controls.groups.controls.find(group => group.value.name === target);

        const item = sourceGroup?.controls.items.at(rowIndex);
        sourceGroup?.controls.items.removeAt(rowIndex);

        const data = item!.value as TableRow;
        this.addRow(targetGroup?.controls.items!, data);
